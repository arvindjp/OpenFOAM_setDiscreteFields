...
#   include "createMesh.H"
	...
	//internalField
	if(internal){
          scalarField & Ifield = tmpField.primitiveFieldRef();
          scalarField Icenter(Ifield.size());
          scalarField newIField(Ifield.size());
          if(direction=="x"){
            Icenter = center.primitiveFieldRef().component(vector::X);
            newIField = interpolateXY(Icenter,posX,fX);
          }	
          else if(direction=="y"){
            Icenter = center.primitiveFieldRef().component(vector::Y);
            newIField = interpolateXY(Icenter,posY,fX);
          }	
          else if(direction=="z"){
            Icenter = center.primitiveFieldRef().component(vector::Z);
	...
	forAll(patchNames,patchNameI)
	  {
	    ...
	    scalarField &Pfield = tmpField.boundaryFieldRef()[patchID];
	...
	//internalField
	if(internal){
          vectorField &IVectorfield = tmpField.primitiveFieldRef();
          vectorField IVectorCenter(IVectorfield.size());
          vectorField newIVectorField(IVectorfield.size());
          IVectorCenter = center.primitiveFieldRef();
          if(direction=="x"){
	...
	forAll(patchNames,patchNameI)
	  {
	    label patchID=mesh.boundaryMesh().findPatchID(patchNames[patchNameI]);
	    vectorField &PVectorfield = tmpField.boundaryFieldRef()[patchID];
